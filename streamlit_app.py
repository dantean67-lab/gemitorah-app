...
# 5. הפעלת המודל המעודכן עם מנגנון שגיאות חכם וסגור היטב
if user_question:
    if "GEMINI_API_KEY" not in st.secrets:
        st.error("⚠️ שגיאה: מפתח ה-API לא הוגדר ב-Secrets של המערכת.")
    else:
        try:
            # הגדרת המפתח והמודל היציב
            ...
            if "429" in error_str or "quota" in error_str or "exhausted" in error_str:
                st.error("⚠️ שגיאה מהשרת (קוד 429) - המכסה החינמית של גוגל הסתיימה.")
                st.info("""
                **💡 המערכת מוכנה! כל שנותר הוא להחליף מפתח API:**
                1. היכנס ל-Google AI Studio דרך חשבון גוגל (Gmail) **אחר** שלא השתמשת בו היום.
                2. לחץ על **Create API Key** והעתק את המפתח החדש.
                3. הדבק את המפתח החדש בלוח הבקרה של Streamlit תחת לשונית ה-**Secrets** במקום המפתח הישן.
                """)
            elif "404" in error_str or "not found" in error_str:
                st.error("⚠️ שגיאה מהשרת (קוד 404)")
                st.info("""
                **💡 המערכת מוכנה! כל שנותר הוא לוודא שהעתקת את מפתח ה-API החדש במלואו:**
                הבעיה היא אך ורק במפתח ה-API שלך. מומלץ להיכנס ל-Google AI Studio, לייצר מפתח API חדש לחלוטין ולעדכן אותו בלוח הבקרה ב-Secrets של Streamlit.
                """)
            else:
                st.error("❌ חלה שגיאה לא צפויה בתקשורת עם שרת ה-AI.")
                st.info(f"פרטי השגיאה למפתחים: {e}")